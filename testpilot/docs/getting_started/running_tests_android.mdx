...
description: "Testpilot allows you to run automated tests on Android devices and emulators. This guide will walk you through setting up and running your first Android test."
---
...
[emulator.wtf](https://emulator.wtf) lets you quickly spin up remote Android emulators, connect over ADB, and run your Android Unit and Testpilot tests on the remote device. You can connect Testpilot to your `emulator.wtf` devices using the following steps:
...
3. Follow the [documentation](https://docs.emulator.wtf/integrations/cli/) to install the `emulator.wtf` CLI
4. Run the following command to create an [`emulator.wtf`](http://emulator.wtf) device and connect it to your local `adb`: `ew-cli start-session --device model=Pixel7,version=34,gpu=auto`
...
Create a test file (e.g., `android-test.pilot.yaml`) with the platform\_config specifying your Android app package:
