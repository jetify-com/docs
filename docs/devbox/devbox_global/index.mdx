...
description:
  "In addition to managing isolated development environments, you can use Devbox as a general
  package manager. Devbox Global allows you to add packages to a global  This is useful for
  installing a standard set of tools you want to use across multiple Devbox Projects."
---
...
```bash
devbox global add ripgrep vim git# Output:ripgrep is now installedvim is now installedgit is now installed
```
...
```bash
devbox global list# Output:* ripgrep* vim* git
```

To remove a global package, use:

```bash
devbox global rm ripgrep# Output:removing 'github:NixOS/nixpkgs/ripgrep'
```
...
- **[Using Flakes with Devbox](/docs/devbox/guides/using_flakes/):** Learn how to install packages
  from Nix Flakes.
...
[Edit this page](https://github.com/jetify-com/devbox/tree/main/docs/app/docs/devbox_global.md)
