...
description:
  "Scripts are shell commands that can be defined in your devbox.json file. They can be executed by
  using the  command. Scripts started with  are launched in a interactive  that terminates once the
  script finishes, or is interrupted by CTRL-C."
---
...
```
"shell": {    "init_hook": "echo \"Hello \"",    "scripts": {        "echo_once": "echo \"World\"",        "echo_twice": [            "echo \"World\"",            "echo \"Again\""        ]    }}
```
...
```bash
$ devbox run echo_onceInstalling nix packages. This may take a while... done.Starting a devbox shell...HelloWorld$ devbox run echo_twiceInstalling nix packages. This may take a while... done.Starting a devbox shell...HelloWorldAgain
```
...
```bash
# Run `lsof -i :80` in your devbox shell in quiet modedevbox run -q lsof -i :80
```
