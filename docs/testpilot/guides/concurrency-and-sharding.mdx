...
description:
  "One way to accelerate execution of a large test suite is to run multiple tests concurrently
  instead of sequentially. Testpilot gives you two independent levers for speeding up a suite:
  running multiple tests at once on a single worker () and slicing the suite across multiple workers
  (). This guide focuses on how Testpilot implements them, what it decides for you, and how to
  combine them without surprises."
---
...
[GitLab Configuration](/docs/testpilot/gitlab) for more details on how to set up sharding in your CI
pipeline.
