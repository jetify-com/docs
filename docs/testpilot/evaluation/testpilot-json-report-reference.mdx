...
description:
  "Complete reference for TestPilot's JSON report structure with schema, field descriptions, and
  enum tables."
...
- **`id`** (string): Unique test identifier. This can be used to correlate tests across runs, even
  if the title or description of the test changes.
...
- **File Locations**: Per-test results saved as `results.json`, aggregated reports contain full
  structure
