...
- **duration** (string): Total execution time in duration format (e.g., "137.221s")
...
- **name** (string): Human-readable action name (e.g., "left click", "screenshot")
...
- **button** (string): Mouse button used (e.g., "left", "right")
- **type** (string): Action type classification (e.g., "click", "type")
...
- **type** (string): Type attribute (e.g., input type)
...
- **Timestamps**: Use ISO 8601 format (e.g., "2025-10-30T10:00:00Z")
- **Durations**: String format with units (e.g., "45.123s", "1m30.5s")
- **Enums**: Always serialized as numeric values in JSON
