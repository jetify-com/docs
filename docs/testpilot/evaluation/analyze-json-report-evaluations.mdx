...
TestPilot generates a `report.json` file for each test in your test run in the `testpilot-out/.internal/<report-id>/<test-id>/results.json` directory. You can view a detailed reference of the report structure [here](./testpilot-json-report-reference.mdx).
